...
if($domoticz) {
	...
	if($deurbel) {schakel($SIDeurbel, 'On');Udevice($SIDeurbel, 0, 'Off');}
	...
	if($SMeldingen=='On') {
		...
		foreach($thermometers as $thermometer) {
			if(${'T'.$thermometer}>$avg + 5 && ${'T'.$thermometer} > 25) {$msg='T '.$thermometer.'='.${'T'.$thermometer}.'°C. AVG='.round($avg,1).'°C';
				if(xcache_get('alerttemp'.$thermometer)<$time-600) {xcache_set('alerttemp'.$thermometer, $time);telegram($msg);ios($msg);if($sms==true) smd($msg);}}
		}
		...
		if($SSD_Zolder_Smoke!= 'Off') {$msg='Rook gedecteerd op de zolder!';if(xcache_get('alertSD_Zolder_Smoke')<$time-90) {xcache_set('alertSD_Zolder_Smoke',$time);telegram($msg);ios($msg);if($sms==true) smd($msg);}}
		if($SDeurbel!='Off') {$msg='Deurbel';if(xcache_get('alertDeurbel')<$time-30) {xcache_set('alertDeurbel',$time);telegram($msg);ios($msg);} Udevice($SIDeurbel,0,'Off');if($SLicht_Hall_Auto=='On') {Schakel($SILicht_Voordeur, 'On');Schakel($SILicht_Inkom, 'On');}
		}}
	
	// 0 = auto, 1 = voorwarmen, 2 = manueel
	//Living
	$Set=16.0;
	$setpoint130 = xcache_get('setpoint130');
	if($setpoint130!=0 && $RTLiving < $time - 43200) {xcache_set('setpoint130',0);$setpoint130=0;}
	if($setpoint130!=2) {
		if($TBuiten<20 && $SThuis=='On' && $SRaamLiving=='Closed') {
	...
	//Badkamer
	$Set=16.0;
	$setpoint111 = xcache_get('setpoint111');
	if($setpoint111!=0 && $RTBadkamer < $time - 3600) {xcache_set('setpoint111',0);$setpoint111=0;}
	if($setpoint111!=2) {
		if($TBuiten<21 && $SThuis=='On') {
			$voorwarmen = voorwarmen($TBadkamer,22,120);
			if(in_array(date('N',$time), array(1,2,3,4,5)) && $time>=(strtotime('6:20')-$voorwarmen) && $time<=(strtotime('7:30'))) $Set=22.0;
	...
	$clouds=xcache_get('clouds');
	if($STLicht_Garage_Auto<$time-10000) {
		if($time>$zonop+$clouds*100+600&&$time<$zononder-$clouds*100+600) {if($SLicht_Garage_Auto=='On'&&$SLicht_Garage=='Off') Schakel($SILicht_Garage_Auto,'Off');}
		else Schakel($SILicht_Garage_Auto,'On');}
	if($STLicht_Hall_Auto<$time-10000) {
		if($time>$zonop+$clouds*100+600&&$time<$zononder-$clouds*20+600) {if($SLicht_Hall_Auto=='On'&& $SLicht_Hall=='Off'&&$SLicht_Inkom=='Off') Schakel($SILicht_Hall_Auto,'Off');}
		else Schakel($SILicht_Hall_Auto,'On');}
	...
	if(xcache_get('openweathermap')<$time-110) {
		...
		if(isset($openweathermap['weather']['0']['icon'])) {
			...
			xcache_set('clouds',$openweathermap['clouds']['all']);
			xcache_set('openweathermap',$time);
			file_get_contents($domoticzurl.'type=command&param=udevice&idx=36&nvalue=0&svalue='.$openweathermap['main']['temp']);
		}
	}
	
	//battery levels
	if(xcache_get('batteries')<$time-21600) {
	...
	if($SThuis=='Off'||$SSlapen=="On") {
		...
		if(xcache_get('setpoint111')!=0) xcache_set('setpoint111',0);
		if(xcache_get('setpoint130')!=0) xcache_set('setpoint130',0);
	}	
