...
if($domoticz) {
	...
	if(xcache_get('domoticzconnection')!='up') {xcache_set('domoticzconnection','up');telegram('Verbinding met Domoticz hersteld');}
	//Zon op / zon onder
	...
	if(($SwitchThuis == 'Off' || $SwitchSlapen == 'On') && $SwitchTimeThuis < $timeout && $SwitchMeldingen == 'On') {
		if($Switchpoort!='Closed') {$msg = 'Poort Open om '.strftime("%H:%M:%S", $SwitchTimepoort);
			if(xcache_get('telegrampoort') < $time - 60) {telegram($msg);xcache_set('telegrampoort', $time); $telegrampoort = $time;}
			if($sms==true && xcache_get('smspoort') < $time - 900) {sms($msg);xcache_set('smspoort', $time); $smspoort = $time;}
			if(xcache_get('iospoort') < $time - 600) {ios($msg);xcache_set('iospoort', $time); $iospoort = $time;}
		...
		if($SwitchAchterdeur!='Closed') {$msg = 'Achterdeur Open om '.strftime("%H:%M:%S", $SwitchTimeAchterdeur);
			if(xcache_get('telegramAchterdeur') < $time - 60) {telegram($msg);xcache_set('telegramAchterdeur', $time); $telegramAchterdeur = $time;}
			if($sms==true && xcache_get('smsAchterdeur') < $time - 900) {sms($msg);xcache_set('smsAchterdeur', $time); $smsAchterdeur = $time;}
			if(xcache_get('iosAchterdeur') < $time - 600) {ios($msg);xcache_set('iosAchterdeur', $time); $iosAchterdeur = $time;}  
			if($SwitchMeldingen == 'On') $SwitchDeurbel='Off';schakel($SwitchidxDeurbel, 'On');Udevice($SwitchidxDeurbel, 0, 'Off');;
		}
		if($SwitchPIR_Garage!='Off' && $SwitchTimeSlapen < $timeout) {$msg = 'Beweging gedecteerd in garage om '.strftime("%H:%M:%S", $SwitchTimePIR_Garage);
			if(xcache_get('telegramPIR_Garage') < $time - 60) {telegram($msg);xcache_set('telegramPIR_Garage', $time); $telegramPIR_Garage = $time;} 
			if($sms==true && xcache_get('smsPIR_Garage') < $time - 120) {sms($msg);xcache_set('smsPIR_Garage', $time); $smsPIR_Garage = $time;}
			if(xcache_get('iosPIR_Garage') < $time - 600) {ios($msg);xcache_set('iosPIR_Garage', $time); $iosPIR_Garage = $time;} 
			if($SwitchMeldingen == 'On') $SwitchDeurbel='Off';schakel($SwitchidxDeurbel, 'On');Udevice($SwitchidxDeurbel, 0, 'Off');;
		}
		if($SwitchPIR_Living!='Off' && $SwitchTimeSlapen < $timeout) {$msg = 'Beweging gedecteerd in living om '.strftime("%H:%M:%S", $SwitchTimePIR_Living);
			if(xcache_get('telegramPIR_Living') < $time - 90) {telegram($msg);xcache_set('telegramPIR_Living', $time); $telegramPIR_Living = $time;} 
			if($sms==true && xcache_get('smsPIR_Living') < $time - 900) {sms($msg);xcache_set('smsPIR_Living', $time); $smsPIR_Living = $time;}
			if(xcache_get('iosPIR_Living') < $time - 600) {ios($msg);xcache_set('iosPIR_Living', $time); $iosPIR_Living = $time;} 
			if($SwitchMeldingen == 'On') $SwitchDeurbel='Off';schakel($SwitchidxDeurbel, 'On');Udevice($SwitchidxDeurbel, 0, 'Off');;
		}
		if($SwitchPIR_Inkom!='Off' && $SwitchTimeSlapen < $timeout) {$msg = 'Beweging gedecteerd in inkom om '.strftime("%H:%M:%S", $SwitchTimePIR_Living);
			if(xcache_get('telegramPIR_Inkom') < $time - 90) {telegram($msg);xcache_set('telegramPIR_Inkom', $time); $telegramPIR_Inkom = $time;} 
			if($sms==true && xcache_get('smsPIR_Inkom') < $time - 900) {sms($msg);xcache_set('smsPIR_Inkom', $time); $smsPIR_Inkom = $time;}
			if(xcache_get('iosPIR_Inkom') < $time - 600) {ios($msg);xcache_set('iosPIR_Inkom', $time); $iosPIR_Inkom = $time;} 
			if($SwitchMeldingen == 'On') $SwitchDeurbel='Off';schakel($SwitchidxDeurbel, 'On');Udevice($SwitchidxDeurbel, 0, 'Off');;
		}
	}
	if($SwitchThuis == 'Off' && $SwitchTimeThuis < $timeout && $SwitchMeldingen == 'On') {
		if($SwitchPIR_Hall!='Off') {$msg = 'Beweging gedecteerd in hall om '.strftime("%H:%M:%S", $SwitchTimePIR_Living);
			if(xcache_get('telegramPIR_Hall') < $time - 90) {telegram($msg);xcache_set('telegramPIR_Hall', $time); $telegramPIR_Hall = $time;} 
			if($sms==true && xcache_get('smsPIR_Hall') < $time - 900) {sms($msg);xcache_set('smsPIR_Hall', $time); $smsPIR_Hall = $time;}
			if(xcache_get('iosPIR_Hall') < $time - 600) {ios($msg);xcache_set('iosPIR_Hall', $time); $iosPIR_Hall = $time;} 
			if($SwitchMeldingen == 'On') $SwitchDeurbel='Off';schakel($SwitchidxDeurbel, 'On');Udevice($SwitchidxDeurbel, 0, 'Off');;
		}
	}
	if($SwitchMeldingen == 'On') {
		...
		foreach($thermometers as $thermometer) {
			if(${'Temp'.$thermometer} > $avgtemperatuur + 5) {$msg = 'Temperatuur '.$thermometer.' = '.${'Temp'.$thermometer}.'. Gemiddelde = '.$avgtemperatuur;
				if(${'telegramtemp'.$thermometer} < $time - 600) {telegram($msg);xcache_set('telegramtemp'.$thermometer, $time); ${'telegramtemp'.$thermometer} = $time;}
				if($sms==true && ${'smstemp'.$thermometer} < $time - 3600) {sms($msg);xcache_set('smstemp'.$thermometer, $time); ${'smstemp'.$thermometer} = $time;}
				if(${'iostemp'.$thermometer} < $time - 1800) {ios($msg);xcache_set('iostemp'.$thermometer, $time); ${'iostemp'.$thermometer} = $time;}
			}
		}
		if($PowerP_Bureel_Tobi > 500 && $telegrampowerbureeltobi < $time - 1800) {$msg = 'Verbruik bureel Tobi = '.$PowerP_Bureel_Tobi;telegram($msg);xcache_set('telegrampowerbureeltobi', $time); $telegrampowerbureeltobi = $time;}
	}
	if($SwitchDeurbel!='Off' && $SwitchMeldingen == 'On' ) {
		$msg = 'Deurbel om '.strftime("%H:%M:%S", $SwitchTimeDeurbel);
		if(xcache_get('telegramsentDeurbel') < $time - 30) telegram($msg);xcache_set('telegramsentDeurbel', $time);
		if(xcache_get('iossentDeurbel') < $time - 30) ios($msg);xcache_set('iossentDeurbel', $time);
		Udevice($SwitchidxDeurbel, 0, 'Off');
	}
	if($Switchpoort!='Closed' && $SwitchTimepoort < $time - 900 && $telegramsentpoort < $time - 900 && $SwitchMeldingen == 'On') {$msg = 'Poort Open sinds '.strftime("%H:%M:%S", $SwitchTimepoort);$telegramsentpoort = time();xcache_set('telegramsentpoort',$telegramsentpoort);telegram($msg);}
	//Living
	$SetLiving = 16.0;
	...
	if($SwitchPIR_Inkom != 'Off' && $SwitchTimePIR_Inkom < $timeout) Schakel($SwitchidxPIR_Inkom, 'Off');
	if($SwitchPIR_Hall != 'Off' && $SwitchTimePIR_Hall < $timeout) Schakel($SwitchidxPIR_Hall, 'Off');
	//Licht Garage 
	if(($SwitchPIR_Garage != 'Off' || $Switchpoort != 'Closed') && $SwitchLicht_Garage == 'Off' && $SwitchLicht_Garage_Auto == 'On') Schakel($SwitchidxLicht_Garage, 'On');
	...
	//Licht Inkom 
	if($SwitchPIR_Inkom != 'Off' && $SwitchLicht_Inkom == 'Off' && $SwitchLicht_Hall_Auto == 'On') Schakel($SwitchidxLicht_Inkom, 'On');
	if($SwitchTimeLicht_Inkom < $time - 180 && $SwitchLicht_Inkom == 'On' && $SwitchLicht_Hall_Auto == 'On') Schakel($SwitchidxLicht_Inkom, 'Off');
	//Licht Hall 
	if($SwitchPIR_Hall != 'Off' && $SwitchLicht_Hall == 'Off' && $SwitchLicht_Hall_Auto == 'On') Schakel($SwitchidxLicht_Hall, 'On');
	...
	if($time > $zonop + 7200 && $time < $zononder - 7200 && $SwitchTimeLicht_Garage_Auto < $time - 14400 && $SwitchLicht_Garage_Auto != 'Off') Schakel($SwitchidxLicht_Garage_Auto, 'Off');
	else if($SwitchTimeLicht_Garage_Auto < $time - 14400 && $SwitchLicht_Garage_Auto != 'On') Schakel($SwitchidxLicht_Garage_Auto, 'On');
	//Licht Hall-Inkom automatisch
	if($time > $zonop + 7200 && $time < $zononder - 7200 && $SwitchTimeLicht_Hall_Auto < $time - 14400 && $SwitchLicht_Hall_Auto != 'Off') Schakel($SwitchidxLicht_Hall_Auto, 'Off');
	else if($SwitchTimeLicht_Hall_Auto < $time - 14400 && $SwitchLicht_Hall_Auto != 'On') Schakel($SwitchidxLicht_Hall_Auto, 'On');
	...
	if($SwitchThuis=='Off' || $SwitchSlapen=="On") {
		...
		if($SwitchLamp_Bureel!='Off' && $SwitchTimeLamp_Bureel < $time) Schakel($SwitchidxLamp_Bureel, 'Off');
		if($SwitchTerras!='Off' && $SwitchTimeTerras < $time) Schakel($SwitchidxTerras, 'Off');
		if($SwitchLicht_Garage!='Off' && $SwitchTimeLicht_Garage < $time) Schakel($SwitchidxLicht_Garage, 'Off');
		if($SwitchLicht_Voordeur!='Off' && $SwitchTimeLicht_Voordeur < $time) Schakel($SwitchidxLicht_Voordeur, 'Off');
		if($SwitchLicht_Inkom!='Off' && $SwitchTimeLicht_Inkom < $time) Schakel($SwitchidxLicht_Inkom, 'Off');
		...
		if($DimmerEettafel!='Off' && $DimmerTimeEettafel < $time) Schakel($DimmeridxEettafel, 'Off');
		if($DimmerZithoek!='Off' && $DimmerTimeZithoek < $time) Schakel($DimmeridxZithoek, 'Off');
	}
	//Buienradar
	if($TextTimeBuienradar < $time - 600) {
		$rains = file_get_contents('http://gps.buienradar.nl/getrr.php?lat=50.892880&lon=3.112568');
		$rains = str_replace('
		', '_', $rains);
		$rains = explode("_", $rains);
		$totalrain = 0;
		foreach($rains as $rain) {
			$totalrain = $totalrain + substr($rain,0,3);
		}
		echo Textdevice($TextidxBuienradar,$totalrain);
	}
	//Refresh Zwave node
	if($SwitchTimeLichtHallZolder > $time - 5) RefreshZwave(20);
	if($SwitchTimeLichtInkomVoordeur > $time - 5) RefreshZwave(23);
	if($SwitchTimeLichtTerrasGarage > $time - 5) RefreshZwave(16);
